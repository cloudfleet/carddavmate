...
function dataToVcard(accountUID, inputUID, inputFilterUID, inputEtag)
{
	var vCardText='';
	var groupCounter=0;
	var tmpvCardEditorRef=$('#vcard_editor');
...
// CATEGORIES
	if((value=tmpvCardEditorRef.find('[data-type="\\%categories"]').find('input[data-type="value"]').val())!='')
	{
...
// NOTE
	if((value=tmpvCardEditorRef.find('[data-type="\\%note"]').find('textarea').val())!='')
	{
...
// X-PHONETIC-FIRST-NAME
	if((value=tmpvCardEditorRef.find('[data-type="ph_firstname"]').val())!='')
	{
...
// X-PHONETIC-LAST-NAME
	if((value=tmpvCardEditorRef.find('[data-type="ph_lastname"]').val())!='')
	{
...
// BDAY
	if((value=tmpvCardEditorRef.find('[data-type="date_bday"]').val())!='')
	{
		var valid=true;
...
// X-ANNIVERSARY
	if((value=tmpvCardEditorRef.find('[data-type="date_anniversary"]').val())!='')
	{
		var valid=true;
		try {var date=$.datepicker.parseDate(globalSessionDatepickerFormat, value)}
		catch (e) {valid=false}
...
// TITLE
	if((value=tmpvCardEditorRef.find('[data-type="title"]').val())!='')
	{
...
// ORG
	value=tmpvCardEditorRef.find('[data-type="org"]').val();
	value2=tmpvCardEditorRef.find('[data-type="department"]').val();
	if(value!='' || value2!='')
	{
...
// X-ABShowAs
	if(tmpvCardEditorRef.find('[data-type="isorg"]').prop('checked'))
	{
...
// PHOTO
	if(!(value=tmpvCardEditorRef.find('[data-type="photo"]').prop('src')).match(RegExp('images/(company|user)\.svg$','')))
	{
...
//			return false;	// vcard UID not present or present more than once
		...
		if(vcard_element!=null && vcard_element.length==1)	// if the N attribute is not present exactly once, vCard is considered invalid
		{
			...
			if(parsed_value[0]!=undefined)
				tmpvCardEditorRef.find('[data-type="family"]').val(vcardUnescapeValue(parsed_value[0])).change();
			if(parsed_value[1]!=undefined)
				tmpvCardEditorRef.find('[data-type="given"]').val(vcardUnescapeValue(parsed_value[1])).change();
			if(parsed_value[2]!=undefined)
				tmpvCardEditorRef.find('[data-type="middle"]').val(vcardUnescapeValue(parsed_value[2])).change();
			if(parsed_value[3]!=undefined)
				tmpvCardEditorRef.find('[data-type="prefix"]').val(vcardUnescapeValue(parsed_value[3])).change();
			if(parsed_value[4]!=undefined)
				tmpvCardEditorRef.find('[data-type="suffix"]').val(vcardUnescapeValue(parsed_value[4])).change();
		...
		if(vcard_element!=null)
		{
			if(vcard_element.length==1)	// if the NOTE attribute is present exactly once
			{
				...
				tmpvCardEditorRef.find('[data-type="\\%note"]').find('textarea').text(vcardUnescapeValue(parsed[4])).change().trigger('autosize.resize');
		...
		if(vcard_element!=null)
		{
			...
			tmpvCardEditorRef.find('[data-type="nickname"]').val(vcardUnescapeValue(parsed[4])).change();
		...
		if(vcard_element!=null)
		{
			...
			tmpvCardEditorRef.find('[data-type="ph_firstname"]').val(vcardUnescapeValue(parsed[4])).change();
		...
		if(vcard_element!=null)
		{
			...
			tmpvCardEditorRef.find('[data-type="ph_lastname"]').val(vcardUnescapeValue(parsed[4])).change();
		...
		if(vcard_element!=null)
		{
			...
			if(valid==true)
			{
				tmpvCardEditorRef.find('[data-type="date_bday"]').val(vcardUnescapeValue($.datepicker.formatDate(globalSessionDatepickerFormat, date))).change();
		...
		if(vcard_element!=null)
		{
			...
			if(valid==true)
			{
				tmpvCardEditorRef.find('[data-type="date_anniversary"]').val(vcardUnescapeValue($.datepicker.formatDate(globalSessionDatepickerFormat, date))).change();
		...
		if(vcard_element!=null)
		{
			...
			tmpvCardEditorRef.find('[data-type="title"]').val(vcardUnescapeValue(parsed[4])).change();
		...
		if(vcard_element!=null)
		{
			...
			if(parsed_value[0]!=undefined)
				tmpvCardEditorRef.find('[data-type="org"]').val(vcardUnescapeValue(parsed_value[0])).change();
			if(parsed_value[1]!=undefined)
				tmpvCardEditorRef.find('[data-type="department"]').val(vcardUnescapeValue(parsed_value[1])).change();
		...
		if(vcard_element!=null)	// if the PHOTO attribute is present more than once, we use the first value
		{
			...
			tmpvCardEditorRef.find('[data-type="photo"]').css('visibility', 'hidden').prop('src', photo).one('load', function(){
...
//				tmpvCardEditorRef.find('[data-type="photo"]').attr('src',parsed[4]);
		...
		else	// use default icons (see X-ABShowAs above)
		{
			if(photo_show_org==true)
				tmpvCardEditorRef.find('[data-type="photo"]').prop('src','images/company.svg');
			else
				tmpvCardEditorRef.find('[data-type="photo"]').prop('src','images/user.svg');
		}
		...
		while((vcard_element=vcard.match(vCard.pre['contentline_ADR']))!=null)
		{
			...
			select_element=tmpvCardEditorRef.find('[data-type="\\%address"]:eq('+element_i+') [data-type="address_type"]').find('[data-type="'+jqueryEscapeSelector(type_values_txt)+'"]');
			if(select_element.length==1)
				select_element.prop('selected',true);
			else if(select_element.length==0)
			{
				// create the missing option
				new_opt=tmpvCardEditorRef.find('[data-type="address_type"] :first-child').clone().attr('data-type',type_values_txt).text(type_values_txt_label).wrap('<div>').parent().html();
			...
			tmpvCardEditorRef.find('[data-type="\\%address"]:eq('+element_i+') [data-addr-field="pobox"]').val(vcardUnescapeValue(parsed_value[0])).change();
			tmpvCardEditorRef.find('[data-type="\\%address"]:eq('+element_i+') [data-addr-field="extaddr"]').val(vcardUnescapeValue(parsed_value[1])).change();
			tmpvCardEditorRef.find('[data-type="\\%address"]:eq('+element_i+') [data-addr-field="street"]').val(vcardUnescapeValue(parsed_value[2])).change();
			tmpvCardEditorRef.find('[data-type="\\%address"]:eq('+element_i+') [data-addr-field="locality"]').val(vcardUnescapeValue(parsed_value[3])).change();
			tmpvCardEditorRef.find('[data-type="\\%address"]:eq('+element_i+') [data-addr-field="region"]').val(vcardUnescapeValue(parsed_value[4])).change();
			tmpvCardEditorRef.find('[data-type="\\%address"]:eq('+element_i+') [data-addr-field="code"]').val(vcardUnescapeValue(parsed_value[5])).change();
		...
		while((vcard_element=vcard.match(vCard.pre['contentline_TEL']))!=null)
		{
			...
			// click to "add" button if not enought data rows present
			if(tmpvCardEditorRef.find('[data-type="\\%phone"]').last().find('[data-type="value"]').val()!='')
				tmpvCardEditorRef.find('[data-type="\\%phone"]').last().find('[data-type="\\%add"]').find('input[type="image"]').click();
			...
			select_element=tmpvCardEditorRef.find('[data-type="\\%phone"]:eq('+element_i+') [data-type="phone_type"]').find('[data-type="'+jqueryEscapeSelector(type_values_txt)+'"]');
			if(select_element.length==1)
				select_element.prop('selected',true);
			else if(select_element.length==0)
			{
				// create the missing option
				new_opt=tmpvCardEditorRef.find('[data-type="phone_type"] :first-child').clone().attr('data-type',type_values_txt).text(type_values_txt_label).wrap('<div>').parent().html();
				// append the option to all element of this type
			...
			tmpvCardEditorRef.find('[data-type="\\%phone"]:eq('+element_i+') [data-type="value"]').val(vcardUnescapeValue(parsed[4])).change();
		...
		while((vcard_element=vcard.match(vCard.pre['contentline_EMAIL']))!=null)
		{
			...
			// click to "add" button if not enought data rows present
			if(tmpvCardEditorRef.find('[data-type="\\%email"]').last().find('[data-type="value"]').val()!='')
				tmpvCardEditorRef.find('[data-type="\\%email"]').last().find('[data-type="\\%add"]').find('input[type="image"]').click();
			...
			select_element=tmpvCardEditorRef.find('[data-type="\\%email"]:eq('+element_i+') [data-type="email_type"]').find('[data-type="'+jqueryEscapeSelector(type_values_txt)+'"]');
			if(select_element.length==1)
				select_element.prop('selected',true);
			else if(select_element.length==0)
			{
				// create the missing option
				new_opt=tmpvCardEditorRef.find('[data-type="email_type"] :first-child').clone().attr('data-type',type_values_txt).text(type_values_txt_label).wrap('<div>').parent().html();
				// append the option to all element of this type
			...
			tmpvCardEditorRef.find('[data-type="\\%email"]:eq('+element_i+') [data-type="value"]').val(vcardUnescapeValue(parsed[4])).change();
		...
		while((vcard_element=vcard.match(vCard.pre['contentline_X-SOCIALPROFILE']))!=null)
		{
			...
			// click to "add" button if not enought data rows present
			if(tmpvCardEditorRef.find('[data-type="\\%profile"]').last().find('[data-type="value"]').val()!='')
				tmpvCardEditorRef.find('[data-type="\\%profile"]').last().find('[data-type="\\%add"]').find('input[type="image"]').click();
			...
			select_element=tmpvCardEditorRef.find('[data-type="\\%profile"]:eq('+element_i+') [data-type="profile_type"]').find('[data-type="'+jqueryEscapeSelector(type_values_txt)+'"]');
			if(select_element.length==1)
				select_element.prop('selected',true);
			else if(select_element.length==0)
			{
				// create the missing option
				new_opt=tmpvCardEditorRef.find('[data-type="profile_type"] :first-child').clone().attr('data-type',type_values_txt).text(type_values_txt_label).wrap('<div>').parent().html();
				// append the option to all element of this type
			...
			tmpvCardEditorRef.find('[data-type="\\%profile"]:eq('+element_i+') [data-type="value"]').val(vcardUnescapeValue(type_values_txt=='twitter' ? '@'+social_user : social_user)).change();
		...
		while((vcard_element=vcard.match(vCard.pre['contentline_URL']))!=null)
		{
			...
			// click to "add" button if not enought data rows present
			if(tmpvCardEditorRef.find('[data-type="\\%url"]').last().find('[data-type="value"]').val()!='')
				tmpvCardEditorRef.find('[data-type="\\%url"]').last().find('[data-type="\\%add"]').find('input[type="image"]').click();
			...
			select_element=tmpvCardEditorRef.find('[data-type="\\%url"]:eq('+element_i+') [data-type="url_type"]').find('[data-type="'+jqueryEscapeSelector(type_values_txt)+'"]');
			if(select_element.length==1)
				select_element.prop('selected',true);
			else if(select_element.length==0)
			{
				// create the missing option
				new_opt=tmpvCardEditorRef.find('[data-type="url_type"] :first-child').clone().attr('data-type',type_values_txt).text(type_values_txt_label).wrap('<div>').parent().html();
				// append the option to all element of this type
			...
			tmpvCardEditorRef.find('[data-type="\\%url"]:eq('+element_i+') [data-type="value"]').val(vcardUnescapeValue(parsed[4])).change();
		...
		while((vcard_element=vcard.match(vCard.pre['contentline_X-ABRELATEDNAMES']))!=null)
		{
			...
			// click to "add" button if not enought data rows present
			if(tmpvCardEditorRef.find('[data-type="\\%person"]').last().find('[data-type="value"]').val()!='')
				tmpvCardEditorRef.find('[data-type="\\%person"]').last().find('[data-type="\\%add"]').find('input[type="image"]').click();
			...
			select_element=tmpvCardEditorRef.find('[data-type="\\%person"]:eq('+element_i+') [data-type="person_type"]').find('[data-type="'+jqueryEscapeSelector(type_values_txt)+'"]');
			if(select_element.length==1)
				select_element.prop('selected',true);
			else if(select_element.length==0)
			{
				// create the missing option
				new_opt=tmpvCardEditorRef.find('[data-type="person_type"] :first-child').clone().attr('data-type',type_values_txt).text(type_values_txt_label).wrap('<div>').parent().html();
				// append the option to all element of this type
			...
			tmpvCardEditorRef.find('[data-type="\\%person"]:eq('+element_i+') [data-type="value"]').val(vcardUnescapeValue(parsed[4])).change();
		...
		while((vcard_element=vcard.match(vCard.pre['contentline_IMPP']))!=null)
		{
			...
			// click to "add" button if not enought data rows present
			if(tmpvCardEditorRef.find('[data-type="\\%im"]').last().find('[data-type="value"]').val()!='')
				tmpvCardEditorRef.find('[data-type="\\%im"]').last().find('[data-type="\\%add"]').find('input[type="image"]').click();
			...
			select_element=tmpvCardEditorRef.find('[data-type="\\%im"]:eq('+element_i+') [data-type="im_type"]').find('[data-type="'+jqueryEscapeSelector(type_values_txt)+'"]');
			if(select_element.length==1)
				select_element.prop('selected',true);
			else if(select_element.length==0)
			{
				// create the missing option
				new_opt=tmpvCardEditorRef.find('[data-type="im_type"] :first-child').clone().attr('data-type',type_values_txt).text(type_values_txt_label).wrap('<div>').parent().html();
				// append the option to all element of this type
			...
			{
				// create the missing option
				new_opt=tmpvCardEditorRef.find('[data-type="im_service_type"] :first-child').clone().attr('data-type',service_type_value).text(service_type_value).wrap('<div>').parent().html();
				// append the option to all element of this type
			...
			tmpvCardEditorRef.find('[data-type="\\%im"]:eq('+element_i+') [data-type="value"]').val(vcardUnescapeValue(parsed[4].replace(RegExp('^[^:]+:'),''))).change();
		...
		tmpvCardEditorRef.attr('data-account-uid',inputContact.accountUID);
		tmpvCardEditorRef.attr('data-url',inputContact.uid);
		tmpvCardEditorRef.attr('data-etag',inputContact.etag);

		// UID is stored also in the Cancel button (for Add -> Cancel support /loading the previous active contact/)
		tmpvCardEditorRef.find('[data-type="cancel"]').attr('data-id',inputContact.uid);
	...
	while((parsed=vcardString.match(vCard.pre['contentline_parse']))!=null)
	{
		...
		switch(parsed[2])
		{
			...
			case 'X-SKYPE':
				...
				params_array[params_array.length]='X-SERVICE-TYPE='+parsed[2].replace(RegExp('^X-|-ID$', 'ig'),'');	// extract the IM type
				params_swc=params_array.sort().join(';');
				attr_value=parsed[4];
			...
			case 'IMPP':
				attr_name=parsed[2];
				params_swc=params_array.sort().join(';').toUpperCase();	// we need upper case here for .unique()
